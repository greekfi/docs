...
itself fails the [init](#option) guard.
...
Self-exercise — the safe path. Delegates to [exerciseFor](#option) with `holder = msg.sender`,
...
`NotYetExpired` on or before the deadline — use [burn](#option) or [exercise](#option) while live.
Caller must be `holder` or authorised via `factory.allowExercise(holder, true)`
(reverts `Unauthorized` otherwise); unlike [exerciseFor](#option) this is harmless — the
...
collateral. Only grant [approveOperator](#factory) to entities you trust to also have
minting rights over your collateral allowance — i.e. audited protocol contracts,
never EOAs and never unaudited integrations.
[approveOperator](#factory) does NOT grant exercise rights ([allowExercise](#factory) is separate) and
does NOT grant redeem rights ([allowRedeem](#factory) is separate). Defaults to `false`;
...
collateral to `exercisor`. Distinct from [approveOperator](#factory): that grants transfer
...
Unlike [allowExercise](#factory), this grant carries no value-extraction risk: the redeemer
...
**⚠ Combination danger.** When combined with [approveOperator](#factory), this flag promotes
the operator's transfer right into a *minting* right against the holder's factory
collateral allowance — see the warning on [approveOperator](#factory). The flag is per-account
